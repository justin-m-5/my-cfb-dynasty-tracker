...
export async function advanceSeason(dynastyId: string, options?: AdvanceSeasonOptions): Promise<{ newYear: number } | null> {
    ...
    if (carryRoster) {
        // Get current roster
        const { data: currentRoster } = await supabase.from('players').select('*').eq('dynasty_id', dynastyId).eq('year_record_id', yearRecord.id)
        ...
        if (currentRoster && currentRoster.length > 0) {
            ...
            const carryForward = currentRoster.filter(p => {
